          ...
          reusingView: (UIView *)view
{
    ...
    {
        NSInteger monthCount = [self.months count];
        NSString *monthName = [[self.months objectAtIndex:(row % monthCount)] lowercaseString];
        NSString *currentMonthName = [[self currentMonthName] lowercaseString];
...
-(NSIndexPath *)todayPath // row - month ; section - year
{
    ...
    NSString *month = [[self currentMonthName] lowercaseString];
    NSString *year  = [self currentYearName];
    
    //set table on the middle
    for(NSString *cellMonth in self.months)
    {
        if([[cellMonth lowercaseString] isEqualToString:month])
        {
